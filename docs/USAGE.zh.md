...
> 下图是**默认的英文界面**。窗口右上角提供语言选择器，可随时切换为中文（见下文「界面语言」），
> 无需重启 Maya。
...
窗口默认以 **English** 启动。**右上角**的选择器可立即把整个界面切换为中文，无需重启 Maya：

* 工具自身的全部文本都会跟随切换：窗口标题、区块标题、按钮、tooltip、占位符、过滤器、
  状态栏、属性详情字段、预览 / 应用报告，以及切换之后产生的日志消息；
* **Maya 数据绝不翻译**：节点名、属性名、plug 名、枚举原始值、类型标签（`Float`、`Double3` 等）、
  技术元数据与异常文本一律保持原样；
* 语言选择通过 `QSettings` 持久化（本平台默认写入 Windows 注册表），下次打开工具自动恢复；
  首次启动没有记录时使用 English；
* 审计日志条目保留写入时的语言（它们是对已发生操作的记录）；空状态与面板框架跟随当前语言。
