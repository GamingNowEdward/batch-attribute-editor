...
| 部分 | 状态 |
| --- | --- |
| Core（遍历 / 类型识别 / 校验 / 搜索 / 写入 / Undo） | ✅ 136 个自动化测试在 Maya 2024.2 的 mayapy 下通过 |
| Undo 粒度（一次 Apply = 一次 Undo） | ✅ 实测验证（150 节点批量、部分失败场景） |
| UI 模块导入与工厂分发 | ✅ 自动化验证 |
...
因此 `test_ui_smoke.py` 里 12 个需要 QWidget 的测试在 batch 模式下被自动跳过，
窗口与交互只能靠真实 GUI 会话确认。
...
但选中后 Value 区会说明"暂不支持编辑"，而不是给出一个会写错的控件。
...
* `MPlug` 的 setter 写入**不进入 Undo 队列**（实测连续 5 次写入后 `undo()` 直接失败），
